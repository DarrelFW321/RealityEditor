...
The welcome screen exposes **Open development room** only in development. It uses sample geometry to build the editor without camera or cloud services. Tap a surface to set a destination; tap an object to select it; use Carry and point/drag across the floor, then Release. While an object is held the panel says whether a release there would be accepted and the object glows amber where it would not; carrying is never blocked by that answer. The Modules panel accepts structured editing commands, shows recent diagnostic events, and runs the **M2**, **M3** and **M4** milestone gates. All three suites also run without a device: `npm run gate` from the repository root, optionally with `M2`, `M3` or `M4` to select one. The M4 suite replays recorded RoomPlan captures from `contracts/fixtures/rooms/captures/`; a development build's **Save capture as fixture** button writes new ones in that exact format. Voice uses the same commands through `/voice/session` when the backend has `OPENAI_API_KEY`.

The live journey is one turn: native RoomPlan measurement starts directly, and the same AR session continues into editing supplying projection, pose, tracking and hand cursor samples. Calibration completes on **coverage**, not on turn angle — the camera-pose stream records which directions were actually observed under good tracking, and a boundary nobody looked at produces a named prompt ("turn toward the west wall") rather than being quietly accepted. A wall the camera never faced arrives as `provenance: 'inferred'` however confident RoomPlan is about it, and the editor says so. Finishing early is always allowed; it is labelled, not blocked. Captured keyframes remain local and are deleted when the session ends. **Their registration into the measured frame is not implemented, so they are not uploaded or falsely labeled as tracked keyframes.**

Calibration requires LiDAR, and the welcome screen says so before starting rather than after a sweep. `ScanCamera.tsx` (the old Vision Camera sweep) is retained but unmounted; see its header comment.
