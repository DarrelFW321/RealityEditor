...
- Minimal voice-first editor surface: Liquid Glass controls (microphone, blueprint, and one context button), voice that starts itself, mute that keeps the session alive, an activity icon driven by the session's own speech events, detached editing so a measured room can be worked on away from the room, and a shaping step after the sweep that gives `offset_wall`, `ceiling_height` and `resize_opening` their first controls. See [the minimal interface](minimal-interface.md).
- Blueprint export: the RSG drawn as a to-scale floor plan and inked to PDF by `UIGraphicsPDFRenderer`, with an on-screen preview from the same sheet. RoomPlan has no 2D or PDF export, so the plan is built from the scene graph — which is also the only way the furniture added in the editor can appear at all. See [blueprint export](blueprint-export.md).
